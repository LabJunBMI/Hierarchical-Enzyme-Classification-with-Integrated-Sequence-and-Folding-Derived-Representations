...
class GNNLayer(nn.Module):
    ...
    def __init__(self, num_hidden, dropout=0.2, num_heads=4):
        ...
        self.attention = TransformerConv(in_channels=num_hidden, out_channels=int(
            num_hidden / num_heads), heads=num_heads, dropout=dropout, edge_dim=num_hidden, root_weight=False)
...
class GraphEC(nn.Module):
    ...
    def __init__(self, node_input_dim, edge_input_dim, hidden_dim, num_layers, dropout, augment_eps, device):
        ...
        self.Graph_encoder = Graph_encoder(
            node_in_dim=node_input_dim, edge_in_dim=edge_input_dim,
            hidden_dim=hidden_dim, seq_in=False, num_layers=num_layers, drop_rate=dropout
        )
        # define the attention layer
        self.attention = Attention(hidden_dim, dense_dim=16, n_heads=4)

        self.input_block = nn.Sequential(
            nn.LayerNorm(1024+9, eps=1e-6), nn.Linear(1024 +
                                                      9, hidden_dim), nn.LeakyReLU()
        )
        attention_heads = 8
        self.output_block = nn.Sequential(
            nn.Linear((attention_heads+1)*hidden_dim, (attention_heads+1)*hidden_dim), nn.LeakyReLU(), nn.LayerNorm(
                (attention_heads+1)*hidden_dim, eps=1e-6), nn.Dropout(dropout), nn.Linear((attention_heads+1)*hidden_dim, 5106)
        )
        num_emb_layers = 2
        self.hidden_block = []
        for i in range(num_emb_layers - 1):
            self.hidden_block.extend([
                nn.LayerNorm(hidden_dim, eps=1e-6), nn.Dropout(
                    dropout), nn.Linear(hidden_dim, hidden_dim), nn.LeakyReLU()
            ])
        ...
        self.ATFC = nn.Sequential(
            nn.Linear(hidden_dim, 64), nn.LeakyReLU(), nn.LayerNorm(
                64, eps=1e-6), nn.Linear(64, attention_heads)  # num_heads
        )
        self.weight = 0.2
    ...
    def padding_ver1(self, x, batch_id, feature_dim):
        ...
        for i in range(batch_size):
            ...
            batch_data[i][:torch.unique(batch_id, return_counts=True)[
                1][i]] = x[len_0:len_1]
    ...
    def forward(self, X, h_V, edge_index, seq, batch_id, batch_data, mask_data, batch_activate_site):
        ...
        # [num_residue, hidden_dim]
        h_V = self.Graph_encoder(h_V, edge_index, h_E, seq, batch_id)
        h_V_stru, mask_baseline = self.padding_ver1(
            h_V.cpu(), batch_id.cpu(), h_V.shape[1])
        ...
        # [B, att_heads,hid] -> [B, att_heads*hid]
        h_V_baseline = torch.flatten(h_V_baseline, start_dim=1)
