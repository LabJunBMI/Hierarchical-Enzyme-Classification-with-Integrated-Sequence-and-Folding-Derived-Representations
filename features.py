...
def get_prottrans(fasta_file, output_path, gpu):
    ...
    for i in tqdm(range(0, len(ID_list), batch_size)):
        ...
        batch_seq_list = [re.sub(r"[UZOB]", "X", sequence)
        ...
        for seq_num in range(len(embedding)):
            ...
            torch.save(seq_emd, output_path +
                       batch_ID_list[seq_num] + '.tensor')
    ...
    for line in pdb_file:
        if (line[0:4].strip() == "ATOM" and int(line[22:26].strip()) != current_pos) or line[0:4].strip() == "TER":
            if current_aa != {}:
                ...
                X.append([
                    current_aa["N"], current_aa["CA"],
                    current_aa["C"], current_aa["O"], R_group
                ])
                current_aa = {}
        ...
        if line[0:4].strip() == "ATOM":
            ...
            if atom != "H":
                ...
                ), line[46:54].strip()]).astype(np.float32)
                current_aa[atom] = xyz
    return np.array(X)


def get_esmfold(fasta_file, output_path):
    os.system('python ./esmfold/esmfold.py -i {fasta} -o {output} --chunk-size 128'.format(
        fasta=fasta_file, output=output_path))
    pdbfasta = {}
...
def get_dssp(fasta, dssp_path, esmfold_path, dssp_save):
    ...
    for i in range(0, len(data_fasta)-1, 2):
        ...
        torch.save(
            torch.tensor(dssp_matrix, dtype=torch.float32),
            "{}{}.tensor".format(dssp_save, ID)
        )
